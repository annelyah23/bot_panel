...
surat=$(curl -sS https://raw.githubusercontent.com/annelyah23/permission/main/ipmini | grep -w $ipes | awk '{print $4}')
...
EOF
    ...
    fun_bot1() {
      [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
        wget -qO- https://raw.githubusercontent.com/annelyah23/bot_panel/BotAPI.sh >/etc/.maAsiss/.Shellbtsss
      }
